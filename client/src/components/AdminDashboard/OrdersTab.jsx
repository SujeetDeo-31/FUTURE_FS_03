...
export default function OrdersTab({ orders = [], onUpdateStatus }) {
  ...
  const [typeFilter, setTypeFilter] = useState('all'); // 'all' | 'dine-in' | 'delivery'
  ...
  const deliveryStages = ['received', 'preparing', 'out for delivery', 'completed', 'cancelled'];
  const dineInStages   = ['received', 'preparing', 'completed', 'cancelled'];
  ...
  return (
    <div className="admin-tab-content active" id="adminTabOrders">
      {/* Controls */}
      <div className="admin-controls" style={{ display: 'flex', gap: '10px', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <input
          type="text"
          className="form-input"
          placeholder="Search by name, email, table or order ID..."
          value={searchQuery}
          onChange={(e) => setSearchQuery(e.target.value)}
          style={{ flex: 1, minWidth: '180px', borderRadius: '8px', padding: '10px 14px', fontSize: '0.85rem' }}
        />
        <select
          className="form-input form-select"
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value)}
          style={{ width: '155px', borderRadius: '8px', padding: '10px 36px 10px 12px', fontSize: '0.85rem' }}
        >
        ...
        <select
          ...
          style={{ width: '175px', borderRadius: '8px', padding: '10px 36px 10px 12px', fontSize: '0.85rem' }}
        >
                ...
                return (
                  <tr key={ord._id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      {isDineIn ? (
                        <div
                          style={{
                            ...
                            fontSize: '0.7rem', fontWeight: 700, marginBottom: '4px', letterSpacing: '0.03em',
                          }}
                        >
                    ...
                    <td style={{ padding: '12px' }}>
                      ...
                      >
                        {(isDineIn ? dineInStages : deliveryStages).map(stage => (
